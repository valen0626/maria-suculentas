...
const ProdCactus = () => {
  const {agregarAlCarro, carroItems, marcarFavorito} = useContext(CarroContexto);

  const [listaProductos, setListaProductos] = useState([])
  const mostrarLista = async () => {
    const productosCollection = collection(dataBase, 'cactus')
  ...
  return (
    ...
    <section className="seccionProd">
      <section className="filtros">
        ...
        <Link to={'/marijuana'} className="linkCategoria">Maria suculentas</Link>
            ...
            <section key={productoItem.nombre} className="producto">
              ...
              <div className="product-info">
                ...
                <div className="btns-prod">
                  <i className="bi bi-heart" onClick={()=> marcarFavorito(productoItem)}></i>
                  <figure onClick={() => agregarAlCarro(productoItem, 1)}>
                    {carroItems.some((item) =>item.nombre === productoItem.nombre)? <img src=".\public\iconos\bt_added_to_cart.svg" alt="add-to-cart"  /> : <img src=".\public\iconos\bt_add_to_cart.svg" alt="add-to-cart" />}
                  </figure>
                </div>
              </div>
