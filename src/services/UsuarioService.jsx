...
export const guardarDireccion = async (idUsuario, direccion) => {
  try {
    const usuarioRef = doc(db, coleccion, idUsuario)
    if (usuarioRef) {
      await updateDoc(usuarioRef, { direccion })
      return {
        status: true,
    ...
    return {
      status: false,
      message: "No se guardó la dirección"
    }
  }
}
